...
@PIPELINES.register_module()
class ZeroModality:
    """Deterministically zero one input stream (Stage 2 unimodal eval).

    Unlike RandomMasking (training-time augmentation with random mode
    selection), this always zeroes the same stream every call, so eval runs
    reproducibly ablate exactly one modality against the pretrained
    dual-stream QFDet weights without any architecture change.
    """
    def __init__(self, zero='thermal'):
        assert zero in ('rgb', 'thermal')
        self.zero = zero

    def __call__(self, results):
        key = results.get('img_fields', ['img1', 'img2'])[0 if self.zero == 'rgb' else 1]
        results[key] = np.zeros(results[key].shape, dtype=results[key].dtype)
        return results
