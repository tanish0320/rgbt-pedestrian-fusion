"""Stage 2 eval-only config: RGB-only ablation of the pretrained QFDet checkpoint.
Thermal stream is zeroed via ZeroModality right after loading (same pretrained
...
test_pipeline = [
    ...
    dict(type='ZeroModality', zero='thermal'),
    dict(
...
data = dict(
    ...
    test=dict(
        ...
        img_prefix=data_root + 'mmdet_data/test/',
        pipeline=test_pipeline))
evaluation = dict(interval=1, metric='bbox')

work_dir = '../../results/preds/eval_rgb_only_qfdet_test'
