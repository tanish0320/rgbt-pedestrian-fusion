...
P2/stride-4 fusion level ONLY, no modality-dropout training. Run on a 3050 (6GB) alongside
the full fusion_v1 (4060) and dropout_only (other 3050) runs, to isolate what the P2 change
alone contributes to the mAP_S delta vs. what modality dropout alone contributes.

Identical to qfdet_r50_fpn_p2_vtuav.py except train_pipeline has NO RandomMasking step —
every other line (neck, anchor strides, nms_pre, optimizer, epochs) is unchanged so the
comparison isolates exactly one variable.

1. neck.start_level 1->0, num_outs 5->6, anchor strides gain a leading 4 (stride-4/P2 level).
   C2 (stride 4) is already computed by the backbone (out_indices includes index 0) but was
   previously discarded by the neck. This directly targets Phase 1's finding: a 32x32px box
   (the small/medium threshold) collapses to a single feature cell at stride 32, and the
   smallest real annotations (~114px^2) are already sub-pixel by then.
...
model = dict(
    ...
    train_cfg=dict(
        ...
        debug=False),
    test_cfg=dict(
        nms_pre=2000,  # raised from baseline's 1000 — ~4x more anchors with P2 added
        min_bbox_size=0,
        score_thr=0.05,
